<script>
  ...
  onMount(() => {
    ...
    const ambientLight = new THREE.AmbientLight(0xffffff, 120);
    scene.add(ambientLight);
    const directionalLight = new THREE.DirectionalLight(0xffffff, 112);
    directionalLight.position.set(0.8, 0.4, 0.9).normalize();
    ...
    loader.load(`${base}/assets/diamond.glb`, (gltf) => {
      ...
      let cameraZ = Math.abs(maxDim / (2 * Math.tan(fov / 4))) * 1.4;
